...
var DEFAULT_EXPC=[
 {k:'seq',on:1},{k:'code',on:1},{k:'name',on:1},{k:'unit',on:1},{k:'price',on:1},{k:'group',on:1},{k:'qty',on:1},{k:'barcode',on:0},{k:'aliases',on:0}
];
...
var LIST_CONFS=load('expConfList',{});
var LIST_COL_DEFS=[
 {k:'code'},{k:'name'},{k:'barcode'},{k:'price'},{k:'qty'},{k:'unit'},{k:'group'}
];
function getListConf(key){
 if(!LIST_CONFS[key]){
  LIST_CONFS[key]=LIST_COL_DEFS.map(function(d){return {k:d.k,on:1}});
 }
 return LIST_CONFS[key];
}
function colTitle(c){
 if(c.k==='sep'){return c.label||'فاصل'}
 if(c.k==='custom'){return c.label||'عمود'}
 return c.label||COLT[c.k]||c.k;
}
window.buildListRows=function(arr,key){
 var conf=getListConf(key);
 return arr.map(function(it,idx){
  var o={};
  conf.forEach(function(c){
   if(!c.on){return}
   var t=colTitle(c);
   if(c.k==='seq'){o[t]=idx+1}
   else if(c.k==='code'){o[t]=it.code}
   else if(c.k==='name'){o[t]=it.name}
   else if(c.k==='barcode'){o[t]=it.barcode}
   else if(c.k==='price'){o[t]=it.price}
   else if(c.k==='qty'){o[t]=it.qty}
   else if(c.k==='unit'){o[t]=it.unit}
   else if(c.k==='group'){o[t]=it.group}
   else if(c.k==='sep'){o[t]=''}
   else if(c.k==='custom'){o[t]=''}
  });
  return o;
 });
};
function itemRowByOrder(it,idx){
 ...
 EXPC.forEach(function(c){
  if(!c.on){return}
  if(c.k==='sep'){o[(c.label||'فاصل')]='';return}
  if(c.k==='custom'){o[(c.label||'عمود')]='';return}
  var key=colTitle(c);
...
function doExport(){
 if(!ITEMS.length){toast('لا توجد بيانات — ارفع ملفًا أولًا',1);return}
 var rows=ITEMS.map(function(it,idx){return itemRowByOrder(it,idx)});
 exportRowsFile('الأصناف',rows);
}
var EDIT=null;
function syncCustomLabelsConf(conf){
 var box=$('mBody');
 if(!box){return}
 box.querySelectorAll('[data-clbl]').forEach(function(inp){
  var idx=Number(inp.getAttribute('data-clbl'));
  if(conf[idx]){conf[idx].label=inp.value}
 });
}
function showColEditor(conf,title,saveFn){
 EDIT={conf:conf,title:title,saveFn:saveFn};
 if(CUR_SCREEN!=='exp'){MSTACK.push(CURRENT_REOPEN)}
 CURRENT_REOPEN=function(){showColEditor(EDIT.conf,EDIT.title,EDIT.saveFn)};
 CUR_SCREEN='exp';
 lockScroll(true);
 ...
 conf.forEach(function(c,i){
  ...
  var label=isCustom?(c.label||''):(colTitle(c));
  ...
  if(isSep){
   ...
   h+='<input type="text" class="inp" data-clbl="'+i+'" value="'+(c.label||'')+'" placeholder="اكتب العنوان..." style="flex:1;font-size:12px;padding:6px">';
  }else{
  ...
  if(isSep||isCustom){
   h+='<button class="btn ghost" data-del="'+i+'">🗑</button>';
  }
  h+='</div>';
 });
 h+='</div>';
 h+='<div class="row" style="margin-top:10px;gap:6px;flex-wrap:wrap">';
 h+='<button class="btn ghost" id="addSepBtn">➕ سطر فارغ</button>';
 h+='<button class="btn ghost" id="addCustBtn">➕ اسم مخصص</button>';
 h+='</div>';
 h+='<div class="row" style="margin-top:8px"><button class="btn" id="expSave">💾 حفظ الترتيب</button></div>';
 $('mBody').innerHTML=h;
 $('modal').hidden=false;
 $('expSave').onclick=function(){
  syncCustomLabelsConf(conf);
  if(saveFn){saveFn()}
  toast('💾 حُفظ الترتيب ✔');
 };
 $('addSepBtn').onclick=function(){
  syncCustomLabelsConf(conf);
  conf.push({k:'sep',on:1,label:''});
  showColEditor(conf,title,saveFn);
 };
 $('addCustBtn').onclick=function(){
  syncCustomLabelsConf(conf);
  conf.push({k:'custom',on:1,label:'عنوان جديد'});
  showColEditor(conf,title,saveFn);
 };
 $('mBody').querySelectorAll('[data-up]').forEach(function(b){
  b.onclick=function(){
   ...
   if(i>0){var t=conf[i-1];conf[i-1]=conf[i];conf[i]=t;showColEditor(conf,title,saveFn)}
  };
 });
 $('mBody').querySelectorAll('[data-dn]').forEach(function(b){
  b.onclick=function(){
   ...
   if(i<conf.length-1){var t=conf[i+1];conf[i+1]=conf[i];conf[i]=t;showColEditor(conf,title,saveFn)}
  };
 });
 $('mBody').querySelectorAll('[data-tg]').forEach(function(b){
  b.onclick=function(){
   ...
   conf[i].on=conf[i].on?0:1;
   showColEditor(conf,title,saveFn);
  };
 });
 $('mBody').querySelectorAll('[data-del]').forEach(function(b){
  b.onclick=function(){
   syncCustomLabelsConf(conf);
...
window.openListColEditor=function(key){
 var conf=getListConf(key);
 showColEditor(conf,'أعمدة قائمة: '+(key||''),function(){
  LIST_CONFS[key]=conf;
  saveLS('expConfList',LIST_CONFS);
  closeModalOne();
 });
};
(function(){
 ...
 row.querySelector('#btnExpConf').onclick=function(){
  showColEditor(EXPC,'ترتيب أعمدة التصدير (يمين→يسار)',function(){saveLS('expConf',EXPC)});
 };
})();
...
function updateSessionBtn(){
 ...
 var b=$('btnResume');
 if(n>0){b.hidden=false;b.textContent='⏳ الجلسات ('+n+')'}
 else{b.hidden=true}
}
function saveCurrentSession(){
 if(!ITEMS.length){toast('لا توجد بيانات لحفظها — ارفع ملفًا أولًا',1);return}
...
(function(){
 var missing=[];
 ['handleFile','autoMap','analyze','showMapModal','showGroupModal','renderChunked'].forEach(function(f){
  if(typeof window[f]!=='function'){missing.push('app1.js')}
 });
 ['openModal','search','listModal','findByBarcode','closeModalOne'].forEach(function(f){
